...
BEGIN
  ...
  -- Usage statistics policies - only create if profiles table exists
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'profiles' AND table_schema = 'public') THEN
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies 
    ...
    END IF;
  ELSE
    -- Create a temporary policy that allows service role access if profiles doesn't exist
    IF NOT EXISTS (
      SELECT 1 FROM pg_policies 
      WHERE tablename = 'usage_statistics' 
      AND policyname = 'Service role can read usage statistics'
    ) THEN
      CREATE POLICY "Service role can read usage statistics"
        ON usage_statistics
        FOR SELECT
        TO service_role
        USING (true);
...
BEGIN
  -- Return empty result if required tables don't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'bookings' AND table_schema = 'public') OR
     NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'parking_spots' AND table_schema = 'public') THEN
    RETURN QUERY SELECT start_date, end_date, 0::NUMERIC, 0::BIGINT, 0::BIGINT, 0::NUMERIC, 0::NUMERIC;
    RETURN;
  END IF;
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'bookings' AND table_schema = 'public') THEN
    RETURN QUERY SELECT 0::BIGINT, 0::NUMERIC, 0::NUMERIC, 0::NUMERIC, 0::BIGINT;
    RETURN;
  END IF;
...
DECLARE
  ...
  parking_spots_exists BOOLEAN;
  bookings_exists BOOLEAN;
BEGIN
  -- Check which tables exist
  SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'profiles' AND table_schema = 'public') INTO profiles_exists;
  SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'parking_spots' AND table_schema = 'public') INTO parking_spots_exists;
  SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'bookings' AND table_schema = 'public') INTO bookings_exists;
  ...
  SELECT 
    target_date,
    CASE WHEN profiles_exists THEN COALESCE((SELECT COUNT(*) FROM profiles), 0) ELSE 0 END,
    COALESCE((SELECT COUNT(DISTINCT user_id) FROM analytics_events WHERE created_at::DATE = target_date), 0),
    CASE WHEN profiles_exists THEN COALESCE((SELECT COUNT(*) FROM profiles WHERE created_at::DATE = target_date), 0) ELSE 0 END,
    CASE WHEN parking_spots_exists THEN COALESCE((SELECT COUNT(*) FROM parking_spots), 0) ELSE 0 END,
    CASE WHEN parking_spots_exists THEN COALESCE((SELECT COUNT(*) FROM parking_spots WHERE is_active = true), 0) ELSE 0 END,
    CASE WHEN bookings_exists THEN COALESCE((SELECT COUNT(*) FROM bookings WHERE created_at::DATE = target_date), 0) ELSE 0 END,
    CASE WHEN bookings_exists THEN COALESCE((SELECT COUNT(*) FROM bookings WHERE created_at::DATE = target_date AND status = 'completed'), 0) ELSE 0 END,
    CASE WHEN bookings_exists THEN COALESCE((SELECT SUM(total_cost) FROM bookings WHERE created_at::DATE = target_date AND status = 'completed'), 0) ELSE 0 END
...
-- Function to update admin policy when profiles table becomes available
CREATE OR REPLACE FUNCTION update_usage_statistics_admin_policy()
RETURNS VOID AS $$
BEGIN
  -- Check if profiles table exists and admin policy doesn't exist yet
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'profiles' AND table_schema = 'public') AND
    ...
    -- Drop the temporary service role policy if it exists
    DROP POLICY IF EXISTS "Service role can read usage statistics" ON usage_statistics;
    
    -- Create the proper admin policy
    CREATE POLICY "Admins can read usage statistics"
      ON usage_statistics
      FOR SELECT
      TO authenticated
...
GRANT EXECUTE ON FUNCTION update_usage_statistics_admin_policy TO service_role;